...
public class Cars {
	...
	private static LocalDateTime getRandomExitTime() {
		return LocalDateTime
				.now()
				.plus((int)(Math.random() * 1000), ChronoUnit.MINUTES);
	}
	...
	private static String getRandomCarNumber() {
		...
		// 1 ~ 999
		carNum.append(String.format("%02d",
				(int)(Math.random() * 999 + 1)));
		carNum.append(symbols.charAt(
				(int)(Math.random() * symbols.length())));
		// 101 ~ 9999
		carNum.append(String.format("%04d",
				(int)(Math.random() * 9899 + 101)));
