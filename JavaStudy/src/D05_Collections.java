...
public class D05_Collections {
	...
	public static void main(String[] args) {
		...
		// Array.sort(arr) : 배열의 모든 값을 크기순으로 정렬한다
		arr1[5] = -99;
		arr1[2] = 30;
		arr1[9] = 30;
		...
		// Arrats.binarySearch(arr1, value) :
		// 원하는 값의 인덱스를 찾아 반환한다 (* 정렬 후에만 작동한다) 
		System.out.println(Arrays.toString(arr1));
		
		System.out.println("찾은 위치: " + Arrays.binarySearch(arr1, -99));
		...
		// Arrays.compare(arr1, arr2) : 두 배열의 크기를 비교해본다
		// 배열 크기 비교시 가장 앞에 있는 값이 우선 순위가 높다
		// 왼쪽 배열이 더 크면 1, 오른쪽 배열이 더 크면 -1, 두 배열이 완전히 같으면 0
		int[] stu1 = {1, 1, 1};
		int[] stu2 = {1, 1, 1};
		...
		Collections.addAll(numList, 60, 20, 30, 99, 55, 1);
		...
		// Set은 정렬할 수 가 없기 때문에 정렬된 모습을 보고싶다면
		// 리스트로 만들어서 정렬해야 한다.
		...
		// Collections.fill(List, size) : 전달한 리스트의 값을 초기화
