...
public class C09_Polymorphism {
	...
	/*
	 	...
	 	- 자식 타입 인스턴스는 이미 내부의 부모 타입을 지니고 있으므로
	 	  부모 타입으로 타입 캐스팅되는 것에는 아무 문제가 없다
	 	- 업캐스팅 된 자식 타입 인스턴스는 원래 가지고 있던 자식 클래스의 기능들을 
	 	  사용할 수 없다
	 	
	 	# 다운 캐스팅
	 	
	 	- 부모 타입을 자식타입으로 만드는 것을 다운 캐스팅이라고 한다
	 	- 일반적으로 부모 타입은 자식 타입이 될 수 없다
	 	- 원래 자식 타입이었다가 업 캐스팅 되야 있는 인스턴스의 경우
	...
	public static void main(String[] args) {
		...
		// 업 캐스팅 되었더라도 오버라이드 된 메서드는 자식에 구현된 내로 사용할 수 있다
		l1.drink();
		w1.drink();
		a1.drink();
		...
		// 원래 트럭이었던 인스턴스를 자동차로 업 캐스팅하여 사용
		...
		System.out.println(c2 instanceof DumpTruck);
...
class Water extends Liquid {
	// 오버라이드 : 상속받은 자식 클래스에서 메서드를 덮어쓰기하는 것
...
class Alcohol extends Liquid {
	@Override
	void drink() {
		System.out.println("술을 마셔서 기분이 좋습니다. 머리도 아픕니다");
	}
}
