...
public class C07_ReferencesType {
	...
	/*
	 	...
	 	- String
	 	- 배열 (배열 타입 변수에는 해당 배열의 시작위치 주소값이 저장되어 있다)
	 	- 모든 클래스들 (여러 타입들을 묶어서 만들어낸 커스텀 타입)
	 	- 참조형 데이터 타입 변수애는 인스턴스의 위치(주소값)가 저장된다
	 	- 참조형 데이터 타입에는 .을 찍어 저장되어 있는 주소를 찾아갈 수 있다
	 */
	public static void main(String[] args) {
		...
		// 참조형 데이터 타입의 경우
		Card c1 = new Card('♡', 10);
		Card c2 = c1; // 얕은 복사 (주소값만 전달하는 것)
		
		Card c3 = new Card(c1.shape, c1.num);
		// 깊은 복사 (실제 동일한 인스턴스를 만들어 내는 것)
		...
		Card c4 = new Card('♠', 99);
		changeNum(c4);
		System.out.println("메서드 실행 후 c4 의 숫자: "+ c4.shape + c4.num);
